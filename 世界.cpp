...
void f(float l) {
    加入缓冲区(颜色缓冲区, { l, l, l, l, l, l, l, l, l, l, l, l });
}
...
void 写颜色(float a, float b, float c, float d) {
    ...
    c = pow(c, 0.7);
    d = pow(d, 0.7);
    加入缓冲区(颜色缓冲区, { a, a, a, b, b, b, c, c, c, d, d, d });
}
...
public:
    ...
    void 放块(int x, int y, int z, int 块号) {
        ...
        // 好像不对劲2333
        if ((0 < 亮度(x, y, z - 1) or between(0 < 亮度(x, y, z + 1), 0,
            15) or between(0 < 亮度(x, y + 1, z), 0, 15) or between(0 < 亮度(x, y -
                1, z), 0, 15) or between(0 < 亮度(x + 1, y, z), 0, 15) or between(
            0 <
            亮度(x - 1, y, z), 0, 15)))
            更新周边亮度(x, y, z);
        绘图重置(x, y, z);
    ...
    void 更新亮度(int x, int y, int z) {
        ...
        if (顶(x, y) < z) {
            ...
            t = max({
                uchar(1), 亮度(x, y, z + 1), 亮度(x, y, z - 1), 亮度(x, y + 1, z), 亮度(
                x, y - 1, z), 亮度(x + 1, y, z), 亮度(x - 1, y, z) }) - 1;
        }
        int 原亮度 = 亮度(x, y, z);
        if (原亮度 != t) {
            亮度(x, y, z) = t;
...
void 画顶(uchar id, int x, int y, int z) {
    加入缓冲区(顶点缓冲区, { 0 + x, 0 + y, 1 + z, 1 + x, 0 + y, 1 + z, 1 + x, 1 + y, 1 +
                   z, 0 + x, 1 + y, 1 + z });
    ...
    float a = (a00 + a01 + a11 + a10) / 60.0, b = (a10 + a11 + a20 + a21) /
        60.0, c = (a11 + a12 + a21 + a22) / 60.0, d = (a01 + a02 + a11 + a12) /
        60.0;
    写颜色(a, b, c, d);
    tex(0, 0, 0, id);
    tex(0, 1, 0, id);
...
void 画底(uchar id, int x, int y, int z) {
    加入缓冲区(顶点缓冲区, { 0 + x, 0 + y, 0 + z, 0 + x, 1 + y, 0 + z, 1 + x, 1 + y, 0 +
                   z, 1 + x, 0 + y, 0 + z });
...
void 画前(uchar id, int x, int y, int z) {
    加入缓冲区(顶点缓冲区, { 0 + x, 1 + y, 0 + z, 0 + x, 1 + y, 1 + z, 1 + x, 1 + y, 1 +
                   z, 1 + x, 1 + y, 0 + z });
    uchar a00 = 主世界.亮度(x - 1, y + 1, z - 1);
...
void 画后(uchar id, int x, int y, int z) {
    加入缓冲区(顶点缓冲区, { 0 + x, 0 + y, 0 + z, 1 + x, 0 + y, 0 + z, 1 + x, 0 + y, 1 +
                   z, 0 + x, 0 + y, 1 + z });
    uchar a00 = 主世界.亮度(x - 1, y - 1, z - 1);
    uchar a01 = 主世界.亮度(x - 1, y - 1, z + 0);
    uchar a02 = 主世界.亮度(x - 1, y - 1, z + 1);
...
void 画左(uchar id, int x, int y, int z) {
    加入缓冲区(顶点缓冲区, { 1 + x, 0 + y, 0 + z, 1 + x, 1 + y, 0 + z, 1 + x, 1 + y, 1 +
                   z, 1 + x, 0 + y, 1 + z });
    uchar a00 = 主世界.亮度(x + 1, y - 1, z - 1);
    uchar a01 = 主世界.亮度(x + 1, y - 1, z + 0);
    uchar a02 = 主世界.亮度(x + 1, y - 1, z + 1);
...
void 画右(uchar id, int x, int y, int z) {
    加入缓冲区(顶点缓冲区, { 0 + x, 0 + y, 0 + z, 0 + x, 0 + y, 1 + z, 0 + x, 1 + y, 1 +
                   z, 0 + x, 1 + y, 0 + z });
    uchar a00 = 主世界.亮度(x - 1, y - 1, z - 1);
    uchar a01 = 主世界.亮度(x - 1, y - 1, z + 0);
    uchar a02 = 主世界.亮度(x - 1, y - 1, z + 1);
